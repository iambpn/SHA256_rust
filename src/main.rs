...
impl SHA256 {
    ...
    fn input_pre_processing(&self, text: &String) -> String {
    ...
    fn process_chunks(&self, chunk: &str) -> [u32; 64] {
        ...
        while i != w.len() {
            let s1 = w[i - 15].rotate_right(7)
                ^ w[i - 15].rotate_right(18)
                ^ ((w[i - 15] as u32) >> 3) as u32;
            let s0 = w[i - 2].rotate_right(17)
                ^ w[i - 2].rotate_right(19)
                ^ ((w[i - 2] as u32) >> 10) as u32;
    ...
    fn get_hash(&mut self, text: &String) -> String {
        ...
        self.binary_msg = self.input_pre_processing(&text[..line_feed].to_string());
...
fn main() {
    ...
    println!("Enter text to generate SHA256 Hash of:");

    let mut text = String::new();
    let res = io::stdin().read_line(&mut text);
